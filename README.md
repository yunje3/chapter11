...
`.streamlit/config.toml`, `.gitignore` 같은 점(`.`)으로 시작하는 파일은 필수 파일이 아닙니다. GitHub 웹 업로드에서 점 파일 업로드가 어렵다면 올리지 않아도 됩니다.

선택 사항으로 샘플 데이터를 저장소에 같이 올리려면 아래처럼 `raw/` 폴더를 만들고 `.mat` 파일을 넣습니다. 정상 파일과 이상 파일을 여러 개 넣어도 앱에서 여러 개를 선택할 수 있습니다.
...
- `저장소 raw 폴더`: GitHub 저장소의 `raw/` 폴더에 들어 있는 `.mat` 파일을 여러 개 선택합니다.
